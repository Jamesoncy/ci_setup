...
(function($) {
		...
		if(settings.goSubmit){
			if(errors == 0){
				...
				if(settings.btn_load != null){
					...
					var pretext = check_form.attr('id');
					lastTxt = settings.btn_load.html();
	...
	$.fn.rWagon = function(options) {
		...
		function links_act(){
			...
			$('#wUpdate'+opt.cart).click(function(){
				...
				// alert(ref);
				update_row(id);
				$('#show-input-row'+opt.cart).show(); //---adds add-new-item link after saving the change/s made after editing a specific row-line
				return false;
			});
	...
	<?php if(isset($load_js)) $this->load->view('js/'.$load_js);?>
    </body>
</html>
